...
app.get('/download/:fileId', async (req, res) => {
  ...
  if (fs.existsSync(cachePath)) {
    ...
    if (Date.now() - stats.mtimeMs < CACHE_EXPIRE) {
      ...
      return res.sendFile(cachePath, err => {
        if (err) {
          console.error("sendFile (cache) error:", err.message);
          if (!res.headersSent) res.status(500).json({ error: true, message: "Gagal mengirim file dari cache" });
        }
      });
    } else {
  ...
  try {
    ...
    res.sendFile(cachePath, err => {
      if (err) {
        console.error("sendFile (fresh) error:", err.message);
        if (!res.headersSent) res.status(500).json({ error: true, message: "Gagal mengirim file hasil unduhan" });
      }
    });
  } catch (error) {
